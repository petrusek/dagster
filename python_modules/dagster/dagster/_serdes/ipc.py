...
def interrupt_on_ipc_shutdown_message(pipe_fd: int) -> Iterator[None]:
    ...
    Args:
        pipe_fd: The file descriptor of the pipe to monitor. Must be readable.
            ...
            descriptor.
    """
    # Send CTRL_C_EVENT on windows. CTRL_BREAK_EVENT can propagate to the entire process group,
    # including the running shell.
    with monitor_ipc_shutdown_pipe(
        pipe_fd, handler=lambda: interrupt_ipc_subprocess_pid(os.getpid(), signal.SIGTERM)
    ):
        yield
